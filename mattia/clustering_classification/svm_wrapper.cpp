...
pcl::SvmTrain::doCrossValidation ()
{
  ...
  svm_cross_validation (&prob_, &param_, nr_fold_, target);
...
pcl::SVM::adaptInputToLibSVM (std::vector<SVMData> training_set, svm_problem &prob)
{
  ...
  for (int i = 0; i < prob.l; i++)
  {
    ...
    if (training_set[i].label != NULL)
    {
      prob.y[i] = *training_set[i].label; // label 0 for noise, 1 for good
...
pcl::SVM::saveProblem (const char *filename, bool labelled = 0)
{
  ...
  for (int j = 0; j < training_set_.size() ; j++)
  {
    ...
    if (labelled)
    {
      assert (training_set_[j].label != NULL);
...
bool
pcl::SvmClassify::loadModel (const char *filename)
{
  SVMModel *out;
...
void
pcl::SvmClassify::predictionTest ()
{
  assert (model_.l != 0);
  assert (prob_.l != 0);
  assert (labelled_training_set_ != 0);
...
void
pcl::SvmClassify::predict ()
{
  assert (model_.l != 0);
  assert (prob_.l != 0);

  if (predict_probability_ && !svm_check_probability_model (&model_))
    fprintf (stderr, "\nModel does not support probabiliy estimates\n");
...
std::vector<double>
pcl::SvmClassify::predict (pcl::SVMData in)
{
  assert (model_.l != 0);
...
void
pcl::SvmClassify::savePrediction (const char *filename)
{
